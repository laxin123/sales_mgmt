...
class Cart(object):
    ...
    def get_total_price(self):
        return sum([Decimal(item['price']) * int(item['quantity']) * int(item['total_day']) for item in self.cart.values()])
    ...
    def filter_cart(self, product):
        carts_iteam = self.List()
        for iteam in carts_iteam:
            if int(iteam['id'])== product.id:
                return iteam